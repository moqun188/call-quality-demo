...
class QualityAnalyzer {
  ...
  _evalCompliance(agentTexts, fullText) {
    ...
    if (politeTotal >= (cfg.checks.politeHigh?.value || 8)) score += cfg.checks.politeHigh?.points || 2;
    else if (politeTotal >= cfg.checks.politeThreshold.value) score += cfg.checks.politeThreshold.points;
  ...
  _evalCommunication(utterances, emotionResult) {
    ...
    const hasProactive = cfg.proactiveKeywords
      ? agentUtterances.some((u) => cfg.proactiveKeywords.some((k) => u.text.includes(k)))
      : false;
